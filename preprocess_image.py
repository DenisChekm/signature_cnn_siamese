...
class PreprocessImage:
    ...
    def preprocess_signature(img: np.ndarray,
        ...
        """
        img = img.astype(np.uint8)
        centered = PreprocessImage.normalize_image(img, canvas_size)
        inverted = 255 - centered
        resized = PreprocessImage.resize_image(inverted, img_size)

        if input_size is not None and input_size != img_size:
            cropped = PreprocessImage.crop_center(resized, input_size)
        else:
            cropped = resized
